...
public final class GLUtility {
    ...
    private static final int[] lastFramebuffer = new int[1];
    ...
    public static void setup() {
        if (glVersion != -1) return; // GL version never changes; only query once
    ...
    public static void push() {
        ...
        if (glVersion >= 330 || GL.getCapabilities().GL_ARB_sampler_objects) {
            ...
            // Unbind MC's sampler so NanoVG's font atlas texture uses its own
            // tex parameters instead of MC's sampler override
            glBindSampler(0, 0);
        }
    ...
    public static void pop() {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, lastFramebuffer[0]);
