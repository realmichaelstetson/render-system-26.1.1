...
public class MultipleComboBoxPropertyElement extends ClickGUIElement {
    private final MultipleComboBoxProperty property;
    private boolean expanded = false;
    private final Animation expandAnim;
    private final Animation boxWidthAnim;
    private final Animation popupWidthAnim;

    public MultipleComboBoxPropertyElement(MultipleComboBoxProperty property) {
    ...
    public void render(
        ...
        String displayStr = property.getFormattedValue();
        float labelEndX = x + 8.0f + labelW;
        float maxAvailableBoxW = Math.max(25.0f, (x + width - 6.0f) - (labelEndX + 6.0f));

        float fontScale = 7.0f;
        if (font != null) {
            float textW = font.getWidth(displayStr, fontScale);
            if (textW + 21.0f > maxAvailableBoxW && property.getValue() != null && property.getValue().size() > 1) {
                displayStr = property.getValue().size() + " Selected";
                textW = font.getWidth(displayStr, fontScale);
            }
    ...
    public void renderPopup(
      ...
      //  CaptureManager.prepareBlurLayer(graphics);
        ...
        for (String opt : options) {
            boolean isSelected = property.isSelected(opt);
            ...
            // White dot indicator on the right for active/selected items
            if (isSelected) {
    ...
    public MultipleComboBoxProperty getProperty() {
        return property;
    }
}
